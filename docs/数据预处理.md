...
- `state`: 卡片当时的学习状态 (0: new, 1: learning, 2: review, 3: relearning)。
...
- `user_id`：记录对应的用户id。
- `time_spent_today`: 计算当日在此次复习之前的累计学习时长。通过按 `day_offset` 分组，对 `duration` 进行累加并位移（shift）实现（不包含当前行）。
- `card_index_today`：这是今天复习的第几张卡。按 `day_offset` 分组后，对每条复习记录在当天内按时间顺序进行排序，并从 1 开始累积计数得到。
- `card_review_count`: 对应特征 `times`。计算该卡片是第几次被复习。通过按 `card_id` 分组计数 (`cumcount() + 1`) 实现。
- `rating_gt1`, `rating_gt2`, `rating_gt3`：将评分等级转化为3维向量。例如，若原`rating`是 2，则编码为 [1, 0, 0]：
    1. 大于1？是 (1)
    2. 大于2？否 (0)
    3. 大于3？否 (0)
...
- `log_time_spent_today`: 原`time_spent_today`
...
- `log_last_duration_on_card`: 该卡片上一次复习的耗时的对数，由`log_duration`移位获得，若无则设为无穷大。

#### Session 上下文特征

- `prev_review_rating`：上一次复习的评分
- `log_prev_review_duration`: 上一次复习的耗时的对数
- `same_card_as_prev`：(Bool) 本次复习的卡片是否与上一次复习是同一张。
